...
if (localStorageType && localStoragePokemons) {
    ...
    const pokemons = await Promise.all(localStoragePokemons.map(async (pokemon) => await createCard(pokemon)));
    container.append(...pokemons.map((p) => p.element));
}
function getPokemonsFromResponse(response) {
...
mainform.addEventListener('submit', async (event) => {
    ...
    const url = type === 'all' ? `${apiUrl}pokemon?limit=100` : `${apiUrl}type/${type}`;
    const response = await fetch(url);
    const data = await response.json();
    pokemonList = getPokemonsFromResponse(data);
    const pokemons = await Promise.all(pokemonList.map(async (pokemon) => await createCard(pokemon)));
...
searchForm.addEventListener('submit', async (event) => {
    ...
    container.innerHTML = ''; // Clear previous results
    const pokemons = await Promise.all(filteredPokemons.map((pokemon) => createCard(pokemon)));
    container.append(...pokemons.map((p) => p.element));
});
// debounce pattern
let timeoutID;
...
async function createCard(pokemon) {
    ...
    <h3>${pokemon.name}</h3>
  `;
    const localStorageExtended = JSON.parse(localStorage.getItem('extendedCards') ?? '[]');
